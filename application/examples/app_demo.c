...
static void on_power_mode_changed(power_mode_t old_mode, power_mode_t new_mode)
{
    ...
    for (int i = 1; i <= 4; i++) {
        ...
        if (app_get_channel_info(i, &info) == 0) {
            printf("  Channel %d: Status=%s, V=%.1fV, I=%.1fmA, P=%.1fmW\n",
                   ...
                   (info.status == CHANNEL_STATUS_OFF) ? "OFF" : "FAULT",
...
static void demo_integration_with_component_layer(void)
{
    ...
    const char* mode_str = (mode == POWER_MODE_NORMAL) ? "NORMAL" :
                           ...
                           (mode == POWER_MODE_PERFORMANCE) ? "PERFORMANCE" : "UNKNOWN";
    ...
    for (int i = 1; i <= 4; i++) {
        ...
        if (app_get_channel_info(i, &info) == 0) {
            ...
            const char* status_str = (info.status == CHANNEL_STATUS_ON) ? "ON" : "OFF";
...
int main(void)
{
    ...
    printf("Architecture:\n");
    printf("  Application -> System -> Component -> Driver -> HAL -> BSP -> Hardware\n\n");
