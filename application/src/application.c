...
{
    switch (status) {
        case CHANNEL_STATUS_OFF:      return "OFF";
        case CHANNEL_STATUS_ON:       return "ON";
        case CHANNEL_STATUS_FAULT:    return "FAULT";
        case CHANNEL_STATUS_OVERLOAD: return "OVERLOAD";
        default:                      return "UNKNOWN";
    }
}
...
int app_generate_status_report(char *buffer, size_t buffer_size)
{
    ...
    offset += snprintf(buffer + offset, buffer_size - offset,
                       "  \"version\": \"%u.%u.%u\",\n",
                       g_app_state.config.version_major,
                       g_app_state.config.version_minor,
...
{
    return system_task_create("MonitorTask", monitor_task_func,
                              NULL, 2048, TASK_PRIORITY_MEDIUM);
}

task_handle_t app_create_control_task(void)
{
    return system_task_create("ControlTask", control_task_func,
                              NULL, 2048, TASK_PRIORITY_HIGH);
}

task_handle_t app_create_communication_task(void)
{
    return system_task_create("CommTask", communication_task_func,
                              NULL, 2048, TASK_PRIORITY_LOW);
}
