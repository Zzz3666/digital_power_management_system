...
json_object_t json_object_create(void)
{
    ...
    json_object_impl_t *obj = (json_object_impl_t*)malloc(sizeof(json_object_impl_t));
    if (obj) {
...
int json_object_add_int(json_object_t obj, const char *key, int value)
{
    ...
    snprintf(impl->pairs[impl->count].value, sizeof(impl->pairs[0].value), "%d", value);
...
int json_object_add_string(json_object_t obj, const char *key, const char *value)
{
    ...
    strncpy(impl->pairs[impl->count].key, key, sizeof(impl->pairs[0].key) - 1);
    snprintf(impl->pairs[impl->count].value, sizeof(impl->pairs[0].value), "\"%s\"", value);
...
int json_object_encode(json_object_t obj, char *buffer, size_t size)
{
    ...
    for (int i = 0; i < impl->count; i++) {
        ...
        if (impl->pairs[i].type == JSON_TYPE_STRING) {
            offset += snprintf(buffer + offset, size - offset, 
                             "\"%s\": %s", impl->pairs[i].key, impl->pairs[i].value);
        } else {
            offset += snprintf(buffer + offset, size - offset, 
                             "\"%s\": %s", impl->pairs[i].key, impl->pairs[i].value);
        }
