...
static ssize_t uart_write(driver_handle_t handle, const void *buffer,
                          size_t size);
...
int uart_device_register(const char *name, const uart_drv_config_t *config) {
  ...
  /* Register device */
  strncpy(uart_devices[uart_device_count].name, name,
          sizeof(uart_devices[0].name) - 1);
  uart_devices[uart_device_count].instance =
      (uint8_t)(config->baudrate > 9600 ? 0 : 1);
