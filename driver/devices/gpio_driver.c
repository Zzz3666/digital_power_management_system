...
static ssize_t gpio_write(driver_handle_t handle, const void *buffer, size_t size);
...
int gpio_device_register(const char *name, const gpio_drv_config_t *config)
{
    ...
    hal_config.mode = (config->mode == GPIO_DRV_MODE_OUTPUT) ? 
    ...
    /* Register device */
    strncpy(gpio_devices[gpio_device_count].name, name, sizeof(gpio_devices[0].name) - 1);
...
static driver_handle_t gpio_open(const char *path, uint32_t flags)
{
    ...
    for (int i = 0; i < gpio_device_count; i++) {
        if (strcmp(gpio_devices[i].name, path) == 0 && gpio_devices[i].initialized) {
